...
<body>
            ...
            <div class="flex w-full h-full justify-center">               
                <div class="flex search rounded-full bg-white cursor-pointer overflow-hidden">
                    ...
                    <div class="text-black">
                        <input type="text" class="flex w-6 h-10 outline-none rounded-full" id="search_input">
                    </div>
                </div>
            </div>
        </div>
    ...
    <main class="w-full bg-gray-100">
        <div class="flex flex-row p-4 bg-zinc-900 text-2xl font-bold text-zinc-400" id="nav">
            <div class="pl-4 pr-4 ml-4 rounded-full bg-white"><a class="all hover:text-black cursor-pointer">All</a></div>
            <div class="pl-4 pr-4 ml-4 rounded-full bg-white"><a class="stories hover:text-black cursor-pointer">Stories</a></div>
            <div class="pl-4 pr-4 ml-4 rounded-full bg-white"><a class="impact hover:text-black cursor-pointer">Impact</a></div>
            <div class="pl-4 pr-4 ml-4 rounded-full bg-white"><a class="company hover:text-black cursor-pointer">Company</a></div>
            <div class="pl-4 pr-4 ml-4 rounded-full bg-white"><a class="newsroom hover:text-black cursor-pointer">Newsroom</a></div>
        </div>
        <div class="flex flex-col p-4 text-3xl font-bold">
            <h3>News</h3>
        </div>
        <div class="grid grid-cols-4 gap-4 p-4 max-sm:grid-cols-1">
            <?php
                $New = new New_class;
                $show = $New ->get_full_New_class();
                $i = 0;
                while($row = mysqli_fetch_assoc($show)){
            ?>
                <div class="news control <?php echo $row['The_Loai'] ?>" id="div_<?php echo $i?>">
                    <a href="news_detail.php?ID=<?php echo $row['ID']?>">
                        <img src="../ADMIN/image_upload/<?php echo $row['Hinh_Anh']?>" alt="">
                        <h1 class="font-bold text-2xl font-sans hover_title cursor-pointer h1_size" id="<?php echo $i?>"><?php echo $row['Ten'] ?></h1>
                    </a>
                </div>
            <?php
                $i++;
            ...
            ?>
            <div class="hidden" id="size"><?php echo $i ?></div>
        </div>
        <div class="">
        </div>
    ...
    <script>
         ...
         for( var i=0 ; i< size ;i++){
            Name[i] = document.getElementById(i).innerText;
         }
         $( ".search" ).click(function() {
            ...
            for(var i = 0;i<size;i++)
            {
                let find = Name[i].includes(search);
                if(find){
                    last_find[index] = Name[i];
                    save_arr[index] = i;
                    index++;
                }
            }
            ...
            for(var i =0; i<save_arr.length;i++){
                $( "#div_"+save_arr[i] ).removeClass( "hidden" );
            }
        });
